...
- **آخر commit:** `a5b5911` — R0.2 + R0.2-fix (نقل طبقة Core).
- **`dev`:** يبقى نقطة رجوع سليمة عند v0.57.4 (`5a7e961`).
- **المرحلة التالية:** R0.3 — نقل Evidence + Risk.
- 3 ملفات SPEC قديمة محذوفة من القرص، staged، تُحفَظ مع commit R0.3.
...
**R0.3** — نقل طبقة Evidence + Risk إلى `Evidence/*.mqh` و`Risk/*.mqh`، مع
بروتوكول "راجع وأصلح أثناء النقل". القبول: يُترجَم بصفر errors، وSummary
يطابق 585.17 / 1104.89. حذف ملفات SPEC الثلاثة المعلّقة يُدمَج مع هذا
الـ commit.
