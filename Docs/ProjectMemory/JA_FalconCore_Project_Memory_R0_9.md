...
## محدّثة بعد R0.9 — إعادة الهيكلة مكتملة — مايو 2026
...
- **آخر commit:** R0.9 — تثبيت وإغلاق؛ **إعادة الهيكلة مكتملة**.
- **`dev`:** يبقى نقطة رجوع سليمة عند v0.57.4 (`5a7e961`).
- **المرحلة التالية:** R1.0 — قرار الاستراتيجية + مواصفة الـ scalp.
...
## 10. المهمة التالية — R1.0 (بداية مسار الاستراتيجية)

إعادة الهيكلة اكتملت. الآن يبدأ المسار الذي من أجله بدأ المشروع:
**حلّ مشكلة الهامش الرقيق في استراتيجية FVG_MICRO.**

**R1.0** — قراران:
1. خيار الاستراتيجية: استراتيجيتنا الخاصّة، أم منهج scalping معروف
   للناسداك. (لا ضمان لاستراتيجية رابحة من أي مصدر؛ القرار بمواصفة
   واختبار على أبريل + مايو.)
2. كتابة `ScalpFvgMicro_SPEC.md` — مواصفة كاملة للاستراتيجية قبل أي كود.
   (شرح استراتيجية S00 مفقود من `operational_rules.md` — R1.0 يسدّ هذا.)

ثم R1.1+ — إعادة بناء الاستراتيجية من الصفر داخل
`Strategies/S00_ScalpFvgMicro/`.

**قيد حرج معروف:** الترتيب الفعلي لسلسلة الـ Apply* هو
`#3،#4،#5،#6،#7،#8،#10،#9،#1،#2،#11،#12` — ليس تصاعديًّا.

**بنود مؤجَّلة (ما بعد إعادة الهيكلة)** — في `Docs/Ideas_Backlog.md`:
تفتيت `ReportWriter` الكبير، macro→input، أرشفة Docs، `CFalconRuntimeState`.
تُراجَع لاحقًا، لا تعطّل مسار الاستراتيجية.
