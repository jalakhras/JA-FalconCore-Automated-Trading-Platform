...
- **آخر commit:** R0.8b — حلّ الحالة المكرّرة + إصلاح FinalWorkingNetUSD.
- **`dev`:** يبقى نقطة رجوع سليمة عند v0.57.4 (`5a7e961`).
- **المرحلة التالية:** R0.8c — تقليل الـ globals (85 → <20).
- الترجمة نظيفة: `0 errors, 0 warnings`؛ الأرقام 585.17/1104.89/157.49.
- مراحل مكتملة: R0.1، R0.2(+fix)، R0.3، R0.4، R0.5(+fix)، R0.5b، R0.6a،
  R0.6b، R0.7a، R0.7b(+fix)، R0.7cd، R0.8a، R0.8b.
- **`EA_VERSION_TAG` يُحدَّث كل مرحلة** (الآن `R0_8b`) — أسماء التقارير
  تحمل رقم المرحلة وتتمايز. بند ثابت في كل مواصفة.
...
**R0.8c** — تقليل سطح الـ globals (`g_*`، حاليًّا ~85 → هدف <20). آخر
خطوات R0.8. متشابكة لكن ليست خطرة كـ R0.7.

**قيود من مراجعة R0.8b (أفكار Backlog #15، #17):**
- نمط الـ forwarder في ReportWriter يصل إلى `g_risk_lifecycle_processor`
  كـ global file-scope — أي تغيير لهذا الـ global يجب أن يحافظ على عمل
  الـ forwarder.
- ترتيب التهيئة في `OnInit` حسّاس: `g_risk_lifecycle_processor.Initialize`
  يجب أن يسبق أي استدعاء تقارير يستخدم `SymbolContext()`.

**قيد حرج معروف:** الترتيب الفعلي لسلسلة الـ Apply* هو
`#3،#4،#5،#6،#7،#8،#10،#9،#1،#2،#11،#12` — ليس تصاعديًّا.

**بعد R0.8c:** R0.9 (تثبيت نهائي — أكبر ملف <600 سطر، فحص شامل) — وبه
تكتمل إعادة الهيكلة. ثم R1.0 (قرار الاستراتيجية + مواصفة الـ scalp).

**ملف `Docs/Ideas_Backlog.md`:** 17 فكرة، 1-6 منفّذة. الباقي يُراجَع في
R0.8c/R0.9 وما بعدها.
