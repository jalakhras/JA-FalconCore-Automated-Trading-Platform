...
- **آخر commit:** `a2a80ee` — R0.6b (مفاتيح إطفاء التقارير التشخيصية).
- **`dev`:** يبقى نقطة رجوع سليمة عند v0.57.4 (`5a7e961`).
- **origin/refactor:** متزامن (تم push).
- **المرحلة التالية:** R0.7 — فصل الـ 12 `Apply*` من ReportWriter.
- الترجمة نظيفة: `0 errors, 0 warnings`.
- مراحل مكتملة: R0.1، R0.2(+fix)، R0.3، R0.4، R0.5(+fix)، R0.5b، R0.6a، R0.6b.
- الجذر منظَّم: `Docs/` (ReviewNotes/Specs/ProjectMemory/Archive) + `StrategyDocs/`.
- مديول التقارير: نواة (TradeLifecycle + Summary) دائمة + تشخيصية خلف 6 مفاتيح مطفأة.
...
- **R0.7** ⏳ فصل الـ 12 `Apply*` من ReportWriter (التصحيح المعماري).
...
**R0.7** — فصل الـ 12 `Apply*` من `CFalconReportWriter` إلى طبقات منطق
حقيقية. التوزيع: 7 → `Risk/`؛ 3 (ApplyPaperRuntime*) → `TradeManagement/`؛
ApplyFvgQualityShadowGuard → `Strategies/S00`؛ ApplyCapitalFlowSourceClassification
→ `Reporting/`. **مرحلة حسّاسة** — تُقسَّم إلى خطوات فرعية. القبول لكل
خطوة: Summary يطابق 585.17/1104.89.

**قيد حرج:** ترتيب استدعاء الـ `Apply*` داخل `RegisterClosedTrade` ذو معنى
— بعضها يقرأ نتيجة ما قبله (DLM ← LossCap، Three-Layer Emergency ←
Runner/SL). التسلسل يُحفَظ بدقّة. خريطة الـ Apply* الكاملة (طول كل دالة +
طبقتها) في `Docs/ReviewNotes/R0_6a_Review_Notes.md` §3.5.
