...
- **آخر commit:** `c3acba4` — R0.4 (نقل Execution + عزل جسر البروكر).
- **`dev`:** يبقى نقطة رجوع سليمة عند v0.57.4 (`5a7e961`).
- **المرحلة التالية:** R0.5 — نقل Router.
- الترجمة نظيفة: `0 errors, 0 warnings`.
- الملف الرئيسي تقلّص: ~17,343 → ~12,559 سطرًا.
...
**R0.5** — نقل طبقة Router إلى `Router/*.mqh`: `CFalconStrategyRegistry`
(~221 سطر) و`CFalconFirstShadowStrategyAdapterShell` (~126 سطر). مرحلة
خفيفة. القبول: يُترجَم بصفر errors/warnings، وSummary يطابق
585.17 / 1104.89.
