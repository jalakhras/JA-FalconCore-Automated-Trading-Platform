...
- **آخر commit:** `ae42caa` — R1.2c (ربط الإغلاق الحقيقيّ لـ S00).
- **`dev`:** يبقى نقطة رجوع سليمة عند v0.57.4 (`5a7e961`).
- **`EA_VERSION_TAG`:** `R1_2c` (`EA_BUILD_TAG = RealisticCloseExecution`).
- **المرحلة التالية:** R1.2d — ربط تسجيل S00 في طبقة TradeLifecycle.
...
## 10. المهمة التالية — R1.2d (ربط تسجيل S00 في TradeLifecycle)

**حالة R1.2c (مكتملة، commit `ae42caa`):** ربط إغلاق S00 الحقيقيّ نجح
— 25/25 صفقة أُغلقت بمنطق S00 (`EXPERT_CLOSE_EXIT`)، لا واحدة بوقف
البروكر. النتيجة الحقيقية **+0.10$** مقابل **−86.42$** في R1.2b
(تحسّن +86.52$). القاعدة محفوظة: `BrokerModifySent=0`،
`RuntimeSLChanged=0`. الإغلاق عبر reverse deal، لا تعديل وقف.

**المشكلة المُشخَّصة:** التقارير المركزية خرجت فارغة — `Summary`
يقول `TotalTrades=0`، و`TradeLifecycle`/`Reconciliation` فارغة.
السبب: S00 تُغلق في دفترها الورقيّ الخاصّ (`CloseActiveTrade →
AppendTradeRow`) ولا تستدعي `RegisterClosedTrade` ولا تبني
`FalconTradeLifecycleRecord`. النظام نفسه يرصدها:
`BROKER_ONLY_TRADE_NOT_IN_LOCK_PAPER_LIFECYCLE`.

**R1.2d يحلّ ذلك:** S00 تبني `FalconTradeLifecycleRecord` (تملأ ≈18
حقلًا إلزاميًّا فقط — الباقي تتولّاه سلسلة الـ 12 Apply داخل
`RegisterClosedTrade`) وتستدعي `g_report_writer.RegisterClosedTrade`
داخل بوّابة S00 الثلاثية. عندها تمتلئ التقارير ويكتمل التقرير
المزدوج (ورقيّ مقابل حقيقيّ). المواصفة: `Docs/Specs/JA_FalconCore_R1_2d_SPEC.md`.

**بعد R1.2d:** تشخيص ضعف S00 في التنفيذ الحقيقيّ — 84% من الصفقات
تُغلق بالوقف (`S00_EXIT_STOP`)، 28% فوز فقط. هل حماية الـ breakeven
تُفعَّل أصلًا؟ يُبنى على التقرير المزدوج المكتمل.
...
**قاعدة ثابتة مكتسَبة:** التحقّق يبدأ بإعادة بناء صريحة (F7) + التأكّد
أن `EA_VERSION_TAG` يطابق المرحلة. أيّ مرحلة تنفيذ حقيقيّ تُختبَر بـ
معيار التحقّق المزدوج: البوّابات مغلقة → FixedLot April يبقى
585.17/1104.89/157.49؛ البوّابات مفتوحة → اختبار 3 أشهر.

**قيد حرج معروف:** ترتيب سلسلة الـ Apply* (إعادة الهيكلة):
`#3،#4،#5،#6،#7،#8،#10،#9،#1،#2،#11،#12` — ليس تصاعديًّا. وترتيب
سلسلة الـ Apply داخل `ApplyTradeLifecycleChain`:
`#3،#4،#5،#6،#7،#8،#10،#9،#1،#2،#11،#12`.

**بنود مؤجَّلة في `Docs/Ideas_Backlog.md`:** نظام تعدّد الاستراتيجيات
(أرقام سحرية مستقلّة + تعميم الـ Bridge)؛ مراجعة دلالة `stage`
لاستراتيجيات التنفيذ الحقيقيّ؛ إزالة الاستراتيجية القديمة؛ مرحلة
تنظيم مدخلات المشروع القديمة؛ استراتيجية التحليل الموجيّ (S0x)؛ تطوير
الـ Runner/الوقف المتحرّك فوق الأساس الرابح؛ إصلاح عمود
`PenetrationDepth`؛ فلتر الوقت لـ S00.
