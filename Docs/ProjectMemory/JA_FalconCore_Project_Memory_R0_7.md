...
- **R0.8** ⏳ تنظيف: globals + الحالة المكرّرة scaffolding + ملفات.
...
**R0.8** — التنظيف. يشمل: (أ) تقليل globals (85 → هدف <20)؛ (ب) معالجة
الحالة المكرّرة scaffolding في `CFalconRiskLifecycleProcessor` مقابل
`CFalconReportWriter` (نسخ `m_totals`/`m_tle_*`/`m_dlm_*`/`m_symbol_context`
— انظر `Docs/Ideas_Backlog.md` #2،#3،#4)؛ (ج) تصحيح توثيق ترتيب السلسلة
في `R0_6a_Review_Notes.md` §3.5؛ (د) حذف الملفات/الكود المؤقّت.

**قيد حرج معروف:** الترتيب الفعلي لسلسلة الـ Apply* (موثّق في
`ApplyTradeLifecycleChain`) هو: `#3،#4،#5،#6،#7،#8،#10،#9،#1،#2،#11،#12`
— **ليس** تصاعديًّا. أي عمل يمسّ السلسلة يحافظ على هذا الترتيب.

**ملف `Docs/Ideas_Backlog.md`:** يحوي 8 أفكار من R0.7 — تُراجَع في R0.8
وما بعدها. لا تُنفَّذ أثناء النقل.
