...
- **آخر commit:** `6c0e3f8` — R0.6a (تفكيك ReportWriter بنيويًّا) + `.gitignore`.
- **`dev`:** يبقى نقطة رجوع سليمة عند v0.57.4 (`5a7e961`).
- **المرحلة التالية:** R0.6b — إضافة مفاتيح إطفاء التقارير التشخيصية.
...
- **R0.6b** ⏳ إضافة مفاتيح إطفاء التقارير التشخيصية (نواة = TradeLifecycle + Summary).
...
**R0.6b** — إضافة مفاتيح `input` للتقارير التشخيصية وإطفاؤها افتراضيًّا.
النواة الدائمة: `TradeLifecycle` + `Summary` فقط. **R0.6b تغيّر السلوك
عمدًا** — التشخيصية تتوقّف. القبول: Summary + TradeLifecycle يبقيان
مطابقَين (585.17/1104.89)؛ التشخيصية لا تُنتَج افتراضيًّا؛ وتشغيل أي مفتاح
يدويًّا يعيد تقريره.

**ملاحظة من مراجعة R0.6a:** ترتيب استدعاء الـ 12 `Apply*` داخل
`RegisterClosedTrade` ذو معنى — بعضها يقرأ نتيجة ما قبله (DLM ← LossCap،
Three-Layer Emergency ← Runner/SL). R0.7 يجب أن تحافظ على التسلسل. خريطة
الـ Apply* الكاملة في `Docs/ReviewNotes/R0_6a_Review_Notes.md` §3.5.
